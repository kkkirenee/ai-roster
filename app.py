...
# --- CSS 視覺重塑：深色底、暖粉紅線、霧霾藍字、名牌感 Banner ---
st.markdown("""
    <style>
...
# 5. 輸入與功能區 (收納在可折疊區域，保持畫面整潔)
with st.expander("🛠️ 設定個人資訊與辨識照片"):
    c1, c2, c3, c4 = st.columns(4)
    with c1: u_name = st.text_input("姓名", value=f["name"], placeholder="姓名", label_visibility="collapsed")
    with c2: u_id = st.text_input("員編", value=f["id"], placeholder="員編", label_visibility="collapsed")
    with c3: u_fleet = st.selectbox("機隊", ["A321", "B738"], label_visibility="collapsed")
    with c4: u_rank = st.selectbox("職級", ["FF", "FY"], label_visibility="collapsed")
    ...
    st.markdown("---")
    uploaded_file = st.file_uploader("Upload Schedule", type=['png', 'jpg', 'jpeg'], label_visibility="collapsed")
    if uploaded_file and st.button("🚀 執行辨識"):
        with st.spinner("AI 正在解析班表內容..."):
            try:
                model = genai.GenerativeModel(model_name='gemini-2.5-flash')
                img = Image.open(uploaded_file)
                # 強化 Prompt，要求嚴格 JSON
                prompt = "Analyze this flight schedule. Return ONLY a valid JSON list of objects with 'title' (flight number only) and 'start' (YYYY-MM-DD). Example: [{'title': '116', 'start': '2026-04-03'}]. Limit to April 2026."
                response = model.generate_content([prompt, img])
                
                # 容錯處理：過濾掉 Markdown 的代碼方塊
                clean_json = re.search(r'\[.*\]', response.text, re.DOTALL)
                if clean_json:
                    events = json.loads(clean_json.group())
                    st.session_state.calendar_events = events
                    st.success("辨識成功！")
                    st.rerun()
                else:
                    st.error("AI 輸出格式不正確，請再試一次。")
